...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams INNER JOIN games ON teams.team_id = games.winner_id or teams.team_id = games.opponent_id WHERE winner_id = team_id ORDER BY name")"
